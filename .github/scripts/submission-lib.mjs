...
export function validateTool(raw, schema) {
  ...
  const enumValue = (key, fallback = "unknown") => {
    const value = string(key);
    if (!value) return fallback;
    if (!schema.properties[key].enum.includes(value)) errors.push(`${key} is invalid.`);
    return value;
  };
  ...
  const tool = { id, name, category, description: string("description"), bestFor: list("bestFor"), strengths: list("strengths"), gettingStarted: list("gettingStarted"), usageNotes: list("usageNotes"), url: canonicalUrl(url), domain: string("domain") || domainFromUrl(url), favicon: validUrl("favicon"), platforms, executionMode: enumValue("executionMode"), signupRequirement: enumValue("signupRequirement"), apiKeyRequirement: enumValue("apiKeyRequirement"), pricing, priceDetails: string("priceDetails"), tags: list("tags"), install: string("install"), start: string("start"), commands, models: list("models"), github: validUrl("github"), docs: validUrl("docs") };
