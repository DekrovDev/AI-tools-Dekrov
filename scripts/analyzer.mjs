...
export function detectCategory(text, allowed) {
  ...
  const rules = [
    ...
    ["app-builders", /\b(app builder|no-code app|vibe coding|build (?:an|apps?) from prompts?)\b/],
    ["orchestration", /\b(orchestrat|multi-agent|agent workflows?|workflow automation|agent team)\b/],
    ["browser-agents", /\b(browser agent|browser automation|web agent)\b/],
    ["automation", /\b(automation platform|automate workflows?|workflow builder)\b/],
    ...
    ["search", /\b(web search|search engine|answer engine)\b/],
    ["meetings", /\b(meeting notes?|meeting assistant|meeting transcription|meeting recorder|notetaker)\b/],
    ["writing", /\b(writing assistant|ai writer|copywriting|content writing|essay writer)\b/],
    ["presentations", /\b(presentation generator|slide deck|ai slides|presentation ai|make presentations?)\b/],
    ["translation", /\b(translation|translate text|ai translator|dubbing|localization)\b/],
    ["education", /\b(ai tutor|homework helper|learning assistant|education ai|study tool)\b/],
    ["customer-support", /\b(customer support|customer service|support agent|helpdesk|support ai)\b/],
    ["sales", /\b(sales ai|sales assistant|prospecting|lead generation|sales copilot)\b/],
    ["marketing", /\b(marketing ai|marketing assistant|ad copy|seo ai|content marketing)\b/],
    ["legal", /\b(legal ai|contract review|legal research|legal assistant)\b/],
    ["finance", /\b(finance ai|financial analysis|stock analysis|trading assistant|accounting ai)\b/],
    ["healthcare", /\b(healthcare ai|medical ai|clinical ai|health assistant|patient)\b/],
    ["productivity", /\b(productivity ai|personal assistant|task management ai)\b/],
    ["design", /\b(ui design|design tool|design ai|design assistant)\b/],
    ["3d", /\b(3d model|3d generation|text-to-3d|ai 3d)\b/],
    ["chat-llm", /\b(chatbot|chat (bot|interface|assistant|model)|language model|llm|conversational ai|assistant)\b/],
    ["voice", /\b(voice ai|speech synthesis|speech-to-text|text-to-speech|tts|transcri)\b/],
    ["music", /\b(music generation|ai music|song generation)\b/],
    ["audio", /\b(podcast|audio editing|audio enhancement|audio production|audio tool)\b/],
    ["image", /\b(image generation|text-to-image|ai image)\b/],
    ["video", /\b(video generation|text-to-video|ai video)\b/],
    ["documents", /\b(document processing|document extraction|ocr)\b/],
    ["observability", /\b(observability|llm tracing|prompt tracing)\b/],
    ["rag", /\b(retrieval augmented|\brag\b|document retrieval)\b/],
    ["databases", /\b(vector database|embedding database)\b/],
    ["inference", /\b(model inference|inference api|inference platform)\b/],
    ["local-ai", /\b(local llm|local ai|self-hosted ai)\b/],
    ["testing", /\b(ai testing|test automation)\b/],
    ["security", /\b(ai security|security testing)\b/],
    ["data-analysis", /\b(data analysis|data analytics)\b/],
    ["dev-tools", /\b(developer tool|sdk|api|middleware|devops|code review tool)\b/],
